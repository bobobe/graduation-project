...
public class chatStart extends JFrame {
    ...
    private Map<String,String> map=null;//用于页面之间传值
    // 变量声明结束
	
    
    //单例模式的结构
	private static chatStart singObj;
    public static chatStart GetInstance(Map<String,String> map){
        if(singObj==null)
            singObj=new chatStart(map);
        return singObj;
    }
    //构造方法，建立一个chatStart的界面实例,同时页面跳转时可以传值
    private chatStart() {
        initComponents();
    ...
    //重载构造方法，用于页面之间传值
    private chatStart(Map<String,String> map) {
    	this.map = map;
        initComponents();
        this.setBounds(400, 200, 280, 380);
        this.update(this.getGraphics());
...
//        jLabel1.setBounds(10, 10, 70, 20);
        if(map!=null)
        {
        	jLabel1.setText("欢迎您,"+map.get("name"));
        }
        getContentPane().add(jLabel1);
        jLabel1.setBounds(10, 10, 70, 20);
